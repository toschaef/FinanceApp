...
const ManageBanks = () => {
  const { email, bankNames, hasItem, state_accounts, state_transactions, state_investments, user_token, dispatch, refreshContext } = useContext(Context);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const dataFromBank = useCallback((bankName) => {
    const accounts = state_accounts.filter(a => a.institution_name === bankName);
    const transactions = state_transactions.filter(t => accounts.some(a => a.account_id === t.account_id));
    const investments = state_investments.filter(i => accounts.some(a => a.account_id === i.account_id));

    return [accounts, transactions, investments];
  }, [state_accounts, state_transactions, state_investments]);
          ...
          {hasItem && (
            <ul className='space-y-4'>
              {bankNames.map((name) => (
                <li
                  key={name}
                  className='bg-white p-4 rounded-lg shadow-md flex items-center gap-4 hover:shadow-md transition'
                >
                  ...
                  <div className='flex-shrink-0 w-[150px] h-[70px]'>
                    <div className='flex items-center justify-center w-full h-full border rounded-sm border-gray-300 overflow-hidden'>
                      <AreaGraph
                        height={70}
                        width={150}
                        accounts={dataFromBank(name)[0]}
                  ...
                  <button
                    ...
                    className='ml-3 w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-red-500 text-red-600 hover:text-white transition'
                  >
